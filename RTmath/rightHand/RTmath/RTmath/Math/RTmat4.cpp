...
{
	matrix[0][3] = vector4.x;
	matrix[1][3] = vector4.y;
	matrix[2][3] = vector4.z;
	matrix[3][3] = vector4.w;

	return (*this);
}

Mat4f &RT::Mat4f::Set(const Vec3f &vector3)
{
	matrix[0][3] = vector3.x;
	matrix[1][3] = vector3.y;
	matrix[2][3] = vector3.z;
}
...
Mat4f &Mat4f::Perspective(const Vec4f &parameters)
{
	...
	for (int i = 0; i < SIZE_4; i++)
	{
		auxM[i][2] = matrix[i][2] * aux[2] - matrix[i][3];
	}
	...
	for (int i = 0; i < SIZE_4; i++)
	{
		for (int j = 0; j < SIZE_4; j++)
		{
			...
			if (i == 3)
			{
				auxM[j][i] = matrix[j][i-1] * aux[i];
			}
			else
			{
				auxM[j][i] = matrix[j][i] * aux[i];
			}
		}
	}
...
Mat4f &Mat4f::LookAt(const Vec3f &eye, const Vec3f &center, const Vec3f &up)
{
	...
	Vec3f right = vc3::Cross(dir, up).Normalize();/*(dir.y * up.z - dir.z * up.y,
													dir.z * up.x - dir.x * up.z,
													dir.x * up.y - dir.y * up.x);*/
	//right.Normalize();

	Vec3f resultUp = vc3::Cross(right, dir);/*(right.y * dir.z - right.z * dir.y,
											   right.z * dir.x - right.x * dir.z,
											   right.x * dir.y - right.y * dir.x);*/

	Mat3f auxM(right.x,    right.y,    right.z,
			   resultUp.x, resultUp.y, resultUp.z,
			  -dir.x,     -dir.y,     -dir.z);
	...
	for (int c = 0; c < SIZE_3; c++)
	{
		...
		{
			for (int j = 0; j < SIZE_3; j++)
				resulM.matrix[i][c] += auxM[j][c] * matrix[i][j];
		}
	}
	for (int i = 0; i < SIZE_4; i++)
	{
		for (int j = 0; j < SIZE_3; j++)
			resulM.matrix[i][3] += matrix[i][j] * auxV[j];

		resulM.matrix[i][3] += matrix[i][3];
	}
